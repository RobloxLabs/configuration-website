...
namespace Roblox.Settings.Site.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            if (!HttpContext.User.Identity.IsAuthenticated) return RedirectToAction("Login");
        ...
        public ActionResult PostLogin(string userName, string password, string returnUrl = "/")
        {
            ...
            if (ActiveDirectoryAccessVerifier.VerifyActiveDirectoryPassword(userName, password, global::Roblox.Settings.Site.Properties.Settings.Default.ActiveDirectoryDomain))
            {
                FormsAuthentication.SetAuthCookie(userName.ToLower(), true);
                AuthenticateThisRequest();
