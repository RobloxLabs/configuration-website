...
$(function () {
    Roblox.ConfigSettings = function (settingsArray) {
        ...
        var updateSettingResultsMulti = function () {
            ...
            for (var ii = 0; ii < namePatterns.length; ii++) {
                (function () {
                    ...
                    if (groupName === "*" && namePattern.length < 2) { return; }
                    if (namePattern.length == 0) { return; } //when doing multisearch don't append "all"
                    ...
                    $.get(url, function (response) {
                        if (requestCount === requestCounter) {
                            ...
                            section.prepend($("<tr class='searchDescription'><td style='border:0;' colspan='4'><b>\"" + namePattern + "\"</b></td></tr>"));
        ...
        function readAnchorText() {
            ...
            if (anchorText && anchorText.length > 1 && anchorText[1].length > 1) {
                anchorText = anchorText[1];
                var split = anchorText.split(";");
                var group = split[0];
                var name = split[1].replace(/,/g, " ");
                var value = split[2].replace(/,/g, " ");
                var target = $("#GroupName option").filter(function () { return $(this).text().trim().toLocaleLowerCase() === group; });
        ...
        function openEditSettingDialog(id, title) {
            var dialog = bootbox.dialog({
                title: title,
                message: $("#settingsDialogTemplateContainer").html(),
                size: 'large',
                backdrop: true,
                buttons: {
                    save: {
        ...
        function isFeatureEnabled(settingName, callback) {
            ...
            if (isFeatureEnabled == null || isFeatureEnabled == undefined) {
                $.get("/Config/GetSetting?" + $.param({ settingGroupName: "AdminWebsite.Properties.Settings", settingName: settingName })).then(function (data) {
        ...
        function getAndSetGuidFieldValuesFromDialogValue() {
            ...
            setGuidFieldValues(newValue.slice(0, 8), newValue.slice(9, 13), newValue.slice(14, 18), newValue.slice(19, 23), newValue.slice(24, 36));
        }       
        ...
        function getAndSetDialogValueFieldFromGuidFields() {
            ...
            if (guidInput1.val().length === 8 &&
                guidInput2.val().length === 4 &&
                guidInput3.val().length === 4 &&
                guidInput4.val().length === 4 &&
                guidInput5.val().length === 12) {
                var value = guidInput1.val() + '-' + guidInput2.val() + '-' + guidInput3.val() + '-' + guidInput4.val() + '-' + guidInput5.val();
        ...
        function init() {
            ...
            $("#settingsResultsDiv .settingsEditButton").on("click", function () {
                ...
                $.getJSON(url + id, function (s) {
                    ...
                    openEditSettingDialog(id, "Edit Setting");
                });
                return false;
            });

            $("#settingsResultsDiv .settingsDeleteButton").on("click", function () {
                if (!Roblox.Admi.SettingsSemaphore.doesUserHoldLockForType(Roblox.Admi.SettingsSemaphore.Types.Platform)) {
                    return false;
                }
                var id = $(this).closest("tr").data("sid");
                var name = $(this).closest("tr").find("div.n").attr("title");
                return showDeleteDialog("/Config/DeleteSettingAjax", id, "Are you sure you want to delete " + name + "?", updateSettingResults);
            });
            ...
            $("#CreateNewSettingButton").click(function () {
                ...
                openEditSettingDialog(-1, "Create Setting");
            ...
            if ($("#GroupName").length !== 0) {
                $("#GroupName").select2({ width: "inherit" });
            }
        ...
        function openEditConnectionDialog(id, title) {
            var dialog = bootbox.dialog({
                title: title,
                message: $("#settingsDialogTemplateContainer").html(),
                size: 'large',
        ...
        $("#connectionStringsResultsDiv a:contains('Edit')").on("click", function () {
        ...
        $("#connectionStringsResultsDiv a:contains('Delete')").on("click", function () {
            var id = $(this).closest("tr").data("id");
        ...
        function addRedisEndpointInput() {
            $("#redisEndpoints #dlgRedisEndpointsInputs").append('<input type="text" class="redisEndpointInput" placeholder="hostname with port or port range. eg. redisserver:8000-8003" />');
        ...
        function addWeightedValueInput() {
            $("#weightedCsv #dlgWeightedCsvInputs").append('<input type="text" class="weightedValueInput" placeholder="Value with weight. eg. foo:40" />');
...
$(function() {
    $("#webSettingHelpDisplayer").click(function () { $(".settingsExplanationHelpText").slideToggle() });
});
