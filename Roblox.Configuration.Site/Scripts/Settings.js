    ...
    Roblox.ConfigSettings = function (settingsArray) {
                ...
                callback: function (result) {
                    if (result) {
                        $.post(postUrl, { id: id }, function (responseMessage) {
                            if (responseMessage) {
                                showAlert("<b>Success</b>: " + responseMessage, "alert-success");
                                updateHandler();
                            }
                            else {
                                showAlert("<b>Error</b>: " + "Delete failed!", "alert-danger");
                            }
                        });
                    }
                }
        ...
        function showMaskedSetting(target) {
            $.ajax({
                ...
                data: { "settingGroupName": target.data("groupname"), "settingName": target.data("settingname") }, // These can't be case sensitive. Why were they?
                ...
                error: function (response) {
                    showAlertDialog("<b>Error</b>", "Error fetching masked value! " + response.status);
                }
            });
        }
        ...
        function init() {
            $(document).on("click", "#alertDiv", function () {
                hideMessage();
            });
